...
#ERROR WITH LLVMLITE!?!?!?!?
#NOTE: UMAP requires numba 0.53 but LFR requires numba 0.48
import umap.umap_ as umap #special dimensionality reduction
...
#or if the dataset is ORIGINAL version:
sub_labels = sublabel(dataset_used, orig_df)
orig_df['sub_labels'] = sub_labels
subgroup_sizes = orig_df['sub_labels'].value_counts()

'----------------------------------------------------------------------------'
#Train-test split for pre-processed dataset
X = prep_df.loc[:, prep_df.columns != 'credit']
X[X.columns[7:59]] = X[X.columns[7:59]].astype('object')
X[X.columns[4]] = X[X.columns[4]].astype('object')
...
#PARTIAL FEATURE SCALING
#scale the numerical columns of the X_train set 
scale = StandardScaler()
#create categorical and numerical variables df
...
#scaling test set with the fitter scaling object
x_cat = X_test.drop(['month','credit_amount','investment_as_income_percentage', 
                      'number_of_credits', 'residence_since', 'people_liable_for'],
                       axis = 1)
x_cat.reset_index(drop=True, inplace=True)
...
cluster_datasets = oversample_german(X_train_new, X_test)

'----------------------------------------------------------------------------'
#predicting the test sets based on strategy 1

X_test_pred = predict_whole_set(cluster_datasets, X_test)

'----------------------------------------------------------------------------'
#predicting the test sets based on strategy 2

X_test_pred2, c_models = predict_per_model(cluster_datasets, kprot, X_test,
                                 categorical_columns)
...
xnum, xcat = split_num_cat(X_test_n, categorical_columns)
costs = labels_cost(xnum, xcat, kprot.cluster_centroids_, euclidean_dissim,
                    matching_dissim, categorical_columns, numerical_columns, 0.5)
X_test_pred3 = predict_w_weights(costs, c_models, X_test)
...
metrics_table = metrics_calculate(X_test, X_test_pred, y_test, prot_attrs, 
                                  privileged_groups, unprivileged_groups)
metrics_table2 = metrics_calculate(X_test, X_test_pred2, y_test, prot_attrs, 
                                  privileged_groups, unprivileged_groups)
metrics_table3 = metrics_calculate(X_test, X_test_pred3, y_test, prot_attrs, 
                                  privileged_groups, unprivileged_groups)
all_results = pd.concat([baseline_stats, metrics_table, metrics_table2,
