...
dataset_orig, privileged_groups, unprivileged_groups = aif_data("german", 2, True)
dataset_extra, privileged_groups, unprivileged_groups = aif_data("german", 2, False)
...
metric_orig = BinaryLabelDatasetMetric(dataset_extra, 
...
#Creating a pipeline to construct a score table
...
#pre-processed
data_df = dataset_orig.convert_to_dataframe()
dataset_df = data_df[0]

#original dataset to combine one of the numerical features if desired
extra_df = dataset_extra.convert_to_dataframe()
extra = extra_df[0]

'----------------------------------------------------------------------------'
#if the imported dataset is Adult!
extra = extra.loc[:,['capital-gain','capital-loss','hours-per-week']]
dataset_df = pd.concat([dataset_df, extra], axis=1, join = 'inner')

#if you also use the original adult data only: to get some of the num. features
dataset_df1 = dataset_df.iloc[:,0:57]
label = dataset_df.iloc[:,98].copy()
dataset_df1['Income Binary'] = label
dataset_df = dataset_df1.drop(['education-num'], axis=1)

#if dataset is original german, del the telephone_num and foreign_worker columns
extra = extra.drop(['telephone=A191','telephone=A192', 'foreign_worker=A201', 'foreign_worker=A202'],
                   axis=1)

'----------------------------------------------------------------------------'
#Note: sublabel func doesn't have compas dataset implemented yet.
sub_labels = sublabel(dataset_used, dataset_df)
dataset_df['sub_labels'] = sub_labels
subgroup_sizes = dataset_df['sub_labels'].value_counts()

#or if the dataset is NOT the pre-processed version:
sub_labels = sublabel(dataset_used, extra)
extra['sub_labels'] = sub_labels
subgroup_sizes = extra['sub_labels'].value_counts()

'----------------------------------------------------------------------------'
#stratify=sub_labels or y?
#Train-test split
X = dataset_df.loc[:, dataset_df.columns != 'credit']
y = dataset_df.loc[:,dataset_df.columns == 'credit'].values
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, 
        
                                                    random_state=42, stratify=sub_labels)
#for NOT pre-processed dataset
X = extra.loc[:, extra.columns != 'credit']
y = extra.loc[:, extra.columns == 'credit'].values
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, 
                                                    random_state=42, stratify=sub_labels)
